...
//check player collision w item
int areColliding(Player player, Player item) {
  if (((player.row + player.height) < item.row) || (player.row > (item.row + item.height)) || ((player.col + player.width) < item.col) || (player.col > (item.col + item.width))) {
    return 0;
  }
  return 1;
}

  //check player collision w bub
int areCollidingBall(Player player, Bubble bub) {
  if (((player.row + player.height) < bub.row) || (player.row > (bub.row + 10))
    || ((player.col + player.width) < bub.col) || (player.col > (bub.col + 10))) {
    return 0;
  }
  return 1;
}
